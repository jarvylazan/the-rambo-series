...
# Override this function when boss dies
func _show_drop_dialogue():
	...
	var lines = drop_dialogue_lines.get(lang, drop_dialogue_lines["en"])
	for msg in lines:
		box.queue_text(msg)

	box.dialogue_finished.connect(
		func ():
			if player:
				player.can_move = true  # Unfreeze movement
	)
